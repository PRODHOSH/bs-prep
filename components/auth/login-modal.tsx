...
export function LoginModal({ open, onOpenChange, onSwitchToSignUp, onSwitchToForgotPassword }: LoginModalProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // Keep loading true — page will redirect. Reset only on failure.
        ...
        <div className="p-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <Button
              ...
              className="w-full h-12 text-sm font-bold bg-white hover:bg-black/5 text-black border border-black/10 rounded-full uppercase tracking-widest transition-all shadow-sm disabled:opacity-60"
            >
              {googleLoading ? (
                <>
                  <svg className="animate-spin w-4 h-4 mr-2.5 text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
