      ...
      <div className="container mx-auto px-4 py-20">
        ...
        <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl max-w-5xl mx-auto overflow-hidden">
          ...
          <CardHeader className="border-b border-slate-200 dark:border-slate-800 pb-6 bg-slate-50 dark:bg-slate-800/50">
            <CardTitle className="text-3xl font-bold text-black">Predict Your Required Scores</CardTitle>
            <p className="text-black/70 text-sm mt-2">Find out what you need in your final exam to achieve your target grade</p>
          </CardHeader>
              ...
              <>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-6">
                    <p className="text-sm text-black/70 mb-6 font-medium">Enter all your scores excluding the final exam (F):</p>
                ...
                {predictions.length > 0 && (
                  <div className="space-y-6">
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-black/70 text-base">Based on your current scores, here's what you need in the final exam</p>
                    </div>
                        ...
                        {predictions.map((pred, index) => (
                          ...
                          >
                            <div className="text-center flex items-center justify-center">
                              <span className="text-4xl font-bold text-black">{pred.grade}</span>
                            </div>
                            <div className="text-center flex items-center justify-center">
                              <div>
                                <p className="text-2xl font-bold text-black dark:text-slate-200">{pred.gradePoints}</p>
                                <p className="text-xs text-black/60 font-medium">points</p>
                              </div>
                            </div>
                            <div className="text-center flex items-center justify-center">
                              {pred.possible ? (
                                <div>
                                  <p className="text-4xl font-bold text-black">{pred.requiredScore.toFixed(1)}%</p>
                                  <p className="text-xs text-black/60 font-medium">out of 100</p>
                                </div>
                              ) : (
                                <p className="text-2xl font-semibold text-black/50 dark:text-black/70">---</p>
                              )}
                            </div>
                            <div className="text-center flex items-center justify-center">
                              {pred.possible ? (
                                pred.requiredScore <= 40 ? (
                                  <span className="px-4 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-black/90 rounded-full text-sm font-semibold">
                                    ✓ Easy
                                  </span>
                                ) : pred.requiredScore <= 70 ? (
                                  <span className="px-4 py-2 bg-slate-200 dark:bg-slate-700 border border-slate-400 dark:border-slate-500 text-black rounded-full text-sm font-semibold">
                                    ⚡ Moderate
                                  </span>
                                ) : pred.requiredScore <= 90 ? (
                                  <span className="px-4 py-2 bg-slate-300 dark:bg-slate-600 border border-slate-500 dark:border-slate-400 text-black dark:text-slate-100 rounded-full text-sm font-semibold">
                                    ⚠️ Challenging
                                  </span>
                                ) : (
                                  <span className="px-4 py-2 bg-slate-400 dark:bg-slate-500 border border-slate-600 dark:border-slate-300 text-white dark:text-black rounded-full text-sm font-semibold">
                                    🔥 Very Hard
                                  </span>
                                )
                              ) : (
                                <span className="px-4 py-2 bg-slate-700/20 border border-slate-700 text-black/50 rounded-full text-sm font-semibold">
                                  ✗ Not Possible
                                </span>
                              )}
                            </div>
                    ...
                    <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-lg">
                      <p className="text-base font-bold text-black mb-4 flex items-center gap-2">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div className="flex items-center gap-3 bg-white dark:bg-slate-900 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                          ...
                          <span className="text-black/90 font-semibold">0-40%: Easy</span>
                        </div>
                        <div className="flex items-center gap-3 bg-white dark:bg-slate-900 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                          <span className="w-4 h-4 rounded-full bg-slate-400 dark:bg-slate-500 shadow-lg"></span>
                          <span className="text-black/90 font-semibold">41-70%: Moderate</span>
                        </div>
                        <div className="flex items-center gap-3 bg-white dark:bg-slate-900 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                          <span className="w-4 h-4 rounded-full bg-slate-500 dark:bg-slate-400 shadow-lg"></span>
                          <span className="text-black/90 font-semibold">71-90%: Challenging</span>
                        </div>
                        <div className="flex items-center gap-3 bg-white dark:bg-slate-900 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                          <span className="w-4 h-4 rounded-full bg-slate-600 dark:bg-slate-300 shadow-lg"></span>
                          <span className="text-black/90 font-semibold">91-100%: Very Hard</span>
                        </div>
                      </div>
                    </div>
                  </div>
